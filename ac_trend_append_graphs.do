...
use "C:\Users\Ariel\Desktop\ITSA_stuff\Compare NW and PRAIS\trend_nw_ac.dta", clear
generate model = "NW"
save "C:\Users\Ariel\Desktop\ITSA_stuff\Compare NW and PRAIS\trend_nw_ac.dta", replace

use "C:\Users\Ariel\Desktop\ITSA_stuff\Compare NW and PRAIS\trend_prais_ac.dta", clear
generate model = "PW"
save "C:\Users\Ariel\Desktop\ITSA_stuff\Compare NW and PRAIS\trend_prais_ac.dta", replace

use "C:\Users\Ariel\Desktop\ITSA_stuff\Compare NW and PRAIS\trend_nw_ac.dta", clear
append using "C:\Users\Ariel\Desktop\ITSA_stuff\Compare NW and PRAIS\trend_prais_ac.dta"
save "C:\Users\Ariel\Desktop\ITSA_stuff\Compare NW and PRAIS\AR(1) Revisions\combined_ac_trend.dta", replace


**** graphs ****

use "C:\Users\Ariel\Desktop\ITSA_stuff\Compare NW and PRAIS\AR(1) Revisions\combined_ac_trend.dta", clear
...
foreach metric in /*power*/ coverage /*bias se rmse*/ {
    ...
    if "`metric'" == "se"       local ylabel "ylabel(0(0.2)0.8, nogrid)"
    ...
    foreach effect_val in 1.25 1.5 2 {
        ...
        // ---- PW panel ----
        twoway ///
            (connected `plotvar' period if model=="PW" & abs(tpost-`effect_val')<0.001 & tcorr_str=="-0.9", sort msize(small)) ///
            (connected `plotvar' period if model=="PW" & abs(tpost-`effect_val')<0.001 & tcorr_str=="-0.6", sort msize(small)) ///
            (connected `plotvar' period if model=="PW" & abs(tpost-`effect_val')<0.001 & tcorr_str=="-0.3", sort msize(small)) ///
            (connected `plotvar' period if model=="PW" & abs(tpost-`effect_val')<0.001 & tcorr_str=="0.0",  sort msize(small)) ///
            (connected `plotvar' period if model=="PW" & abs(tpost-`effect_val')<0.001 & tcorr_str=="0.3",  sort msize(small)) ///
            (connected `plotvar' period if model=="PW" & abs(tpost-`effect_val')<0.001 & tcorr_str=="0.6",  sort msize(small)) ///
            (connected `plotvar' period if model=="PW" & abs(tpost-`effect_val')<0.001 & tcorr_str=="0.9",  sort msize(small)) ///
            , `yref' ///
        ...
        // ---- NW panel ----
        twoway ///
            (connected `plotvar' period if model=="NW" & abs(tpost-`effect_val')<0.001 & tcorr_str=="-0.9", sort msize(small)) ///
            (connected `plotvar' period if model=="NW" & abs(tpost-`effect_val')<0.001 & tcorr_str=="-0.6", sort msize(small)) ///
            (connected `plotvar' period if model=="NW" & abs(tpost-`effect_val')<0.001 & tcorr_str=="-0.3", sort msize(small)) ///
            (connected `plotvar' period if model=="NW" & abs(tpost-`effect_val')<0.001 & tcorr_str=="0.0",  sort msize(small)) ///
            (connected `plotvar' period if model=="NW" & abs(tpost-`effect_val')<0.001 & tcorr_str=="0.3",  sort msize(small)) ///
            (connected `plotvar' period if model=="NW" & abs(tpost-`effect_val')<0.001 & tcorr_str=="0.6",  sort msize(small)) ///
            (connected `plotvar' period if model=="NW" & abs(tpost-`effect_val')<0.001 & tcorr_str=="0.9",  sort msize(small)) ///
            , `yref' ///
    ...
    graph export "C:\Users\Ariel\Desktop\ITSA_stuff\Compare NW and PRAIS\AR(1) Revisions\combined_trend_ac_`metric'.png" ///
        , replace width(3000)
}